...
class CursoList(Resource):
    ...
    def post(self):
        ...
        if validate:
            ...
            formacao_curso = formacao_service.listar_formacao(formacao)
            if formacao_curso is None:
                return make_response(jsonify("Formacação não encontrada"), 404)
...
class CursoDetail(Resource):
    def get(self, id):
        curso = curso_service.listar_curso(id)
        if curso is None:
    ...
    def put(self, id):
        curso_bd = curso_service.listar_curso(id)
        if curso_bd is None:
            return make_response(jsonify("Curso não encontrado"), 404)
        ...
        if validate:
            ...
            curso_atualizado = curso_service.listar_curso(id)
